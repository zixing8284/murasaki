...
The original playground asset tree mixed icon-sized UI images with larger decorative or content images. Files such as quick-launch icons and desktop icons ended up under `public/img/`, while Windows 98 `.ico` assets live under `public/icons/`. That made it harder for humans and AI agents to know where to add or search for semantic icons.
...
- source icon packs such as `icons/windows98-icons/ico/`
- icon-sized PNG/GIF/SVG fallbacks only when no suitable Windows 98 `.ico` exists
...
1. `packages/playground/public/icons/windows98-icons/ico/` for Win98 system/app/file/action icons.
2. `packages/playground/public/icons/` for curated or app-specific icon PNGs that have no suitable `.ico` replacement.
...
Most shell icons should use `icons/windows98-icons/ico/`. Custom PNG icons are acceptable under `public/icons/` only for app identity or product-specific artwork that the Windows 98 icon pack does not cover, such as Webamp.
