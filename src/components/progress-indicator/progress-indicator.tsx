...
export function ProgressIndicator({
  ...
  // Calculate number of tiles based on container width and progress value
  const updateTileCount = useCallback(() => {
    if (!tilesWrapperRef.current || variant !== "tile") {
      return;
    }
    // Use requestAnimationFrame to batch layout reads
    window.requestAnimationFrame(() => {
      if (!tilesWrapperRef.current) return;
      const containerWidth =
        tilesWrapperRef.current.getBoundingClientRect().width;
      const progressWidth = (clampedValue / 100) * containerWidth;
      // Use Math.ceil to ensure tiles fill the progress area completely
      // Extra tiles beyond container width will be clipped by overflow-hidden
